...
from collectors.seoul_citydata import build_url, collect_and_store, fetch, load_areas, save_raw
...
def test_save_raw_creates_jsonl_file_with_single_line_when_new(tmp_path: Path):
    payload = {"foo": "bar"}
    ...
    assert result_path == tmp_path / "2026-06-26" / "광화문·덕수궁.jsonl"
    lines = result_path.read_text(encoding="utf-8").splitlines()
    assert len(lines) == 1
    assert json.loads(lines[0]) == {"fetched_at": "2026-06-26T15:18:00", "payload": payload}


def test_save_raw_appends_line_when_file_already_exists(tmp_path: Path):
    area_name = "광화문·덕수궁"
...
def test_collect_and_store_keeps_local_save_when_r2_upload_fails(tmp_path: Path):
    ...
    assert results[0]["local_path"] == tmp_path / "2026-06-26" / "강남역.jsonl"
